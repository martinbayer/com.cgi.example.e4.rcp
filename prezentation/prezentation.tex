...
\end{frame}
\begin{frame}{Extensions (deprecated)}
	\begin{itemize}
		\item built on \str{OSGi} extensions
		\item plugins define (\ttt{plugin.xml})
			\begin{enumerate}
				\item \str{extension point} -- what \str{extensions} must provide
					\begin{itemize}
						\item attributes -- typed (XML Schema)
						\item classes -- implements/extends
					\end{itemize}
				\item \str{extension}(s) -- provide values/class
			\end{enumerate}
		\item plugins read extensions and obtain instances \eg
		\item concrete implementation not accessible (dependency)
		\item DI problematic (\ttt{IExecutableExtension})
	\end{itemize}
\end{frame}
